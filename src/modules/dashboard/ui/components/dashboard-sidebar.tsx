            ...
            <SidebarMenu>
              {firstSection.map((item) => (
                <Collapsible
                  key={item.href}
                  asChild
                  defaultOpen={false} // Set to true if you want it open by default
                  className="group/collapsible"
                >
                  <SidebarMenuItem>
                    <CollapsibleTrigger asChild>
                      <SidebarMenuButton className={cn("h-10 hover:linear-to-r/oklch border border-transparent hover:border-[#5D6B68]/10 from-sidebar-accent from-5% via-30% via-sidebar/50 to-sidebar/50")} tooltip={item.label}>
                        <item.icon />
